...
final class DriveModelTests: XCTestCase {
    ...
    @MainActor func testUnmountDeviceTimeout() throws {
        print("\n🧪 Test: unmountDevice timeout behavior")
        print("=" * 50)
        
        // Create device with non-existent mount point to trigger timeout
        let timeoutDevice = Drive(
            name: "Timeout Test Device",
            mountPoint: "/dev/disk999", // Non-existent device
            size: 0,
            isRemovable: true,
            isSystemDrive: false,
            isReadOnly: false,
            mediaUUID: nil,
            mediaName: "TIMEOUT_TEST",
            vendor: "Test",
            revision: "1.0"
        )
        
        print("🔧 Testing timeout with non-existent device: \(timeoutDevice.mountPoint)")
        
        let startTime = Date()
        let result = timeoutDevice.unmountDevice()
        let executionTime = Date().timeIntervalSince(startTime)
        
        print("⏱️ Execution time: \(String(format: "%.2f", executionTime))s")
        
        // Should timeout within 10 seconds + buffer
        XCTAssertLessThan(executionTime, 12.0, "unmountDevice should timeout within 12 seconds")
        XCTAssertFalse(result, "unmountDevice should return false for non-existent device")
        
        print("✅ Timeout behavior working correctly")
    }
    
    @MainActor func testMountDeviceTimeout() throws {
        print("\n🧪 Test: mountDevice timeout behavior")
        print("=" * 50)
        
        // Create device with non-existent mount point
        let timeoutDevice = Drive(
            name: "Mount Timeout Test",
            mountPoint: "/dev/disk998", // Non-existent device
            size: 0,
            isRemovable: true,
            isSystemDrive: false,
            isReadOnly: false,
            mediaUUID: nil,
            mediaName: "MOUNT_TIMEOUT_TEST",
            vendor: "Test",
            revision: "1.0"
        )
        
        print("🔧 Testing mount timeout with non-existent device: \(timeoutDevice.mountPoint)")
        
        let startTime = Date()
        let result = timeoutDevice.mountDevice()
        let executionTime = Date().timeIntervalSince(startTime)
        
        print("⏱️ Execution time: \(String(format: "%.2f", executionTime))s")
        
        // Should timeout within 10 seconds + buffer
        XCTAssertLessThan(executionTime, 12.0, "mountDevice should timeout within 12 seconds")
        XCTAssertFalse(result, "mountDevice should return false for non-existent device")
        
        print("✅ Mount timeout behavior working correctly")
    }
